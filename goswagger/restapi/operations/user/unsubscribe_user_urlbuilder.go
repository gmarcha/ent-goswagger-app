...
func (o *UnsubscribeUserURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{userId}/events/{eventId}"

	eventID := o.EventID
	if eventID != "" {
		_path = strings.Replace(_path, "{eventId}", eventID, -1)
	} else {
		return nil, errors.New("eventId is required on UnsubscribeUserURL")
	}
	...
	if userID != "" {
		...
		return nil, errors.New("userId is required on UnsubscribeUserURL")
	}
...
func (o *UnsubscribeUserURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UnsubscribeUserURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UnsubscribeUserURL")
	}
