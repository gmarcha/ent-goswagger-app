...
- Repository architecture follows some conventions:
  ...
  - `/internal/` contains all other Go packages used in our applications:
    ...
    - `/ent/` contains generated code from entgo (data schemas defined in `/ent/schema/`
        serve as data models to code generation);
    - `/goswagger/` contains swagger generated server code (configuration should be
        write in `/goswagger/restapi/configure_tutor.go`);
...
2. Code and assets generation is performed with entgo, a bash script and goswagger. First it is necessary to generate Go data models from ent schemas (it also generates an openapi specification containing data definitions in `/ent/` folder). Then a new swagger specification should be generated by merging the configuration specification and the data definitions specification. The new specification is a read-only file belonging to `/docs/` and serving as an API reference and documentation. Finally this specification serves as a model to perform server code generation with goswagger.

3. Handlers and authentication mechanisms need to be implemented by developers. Generic handlers are implemented as modules, i.e. packages which gather all functionalities (handlers, services, validation) related to an object in the same folder, rather than splitting them trough different packages. Login and callback handlers should be configure to perform an OAuth handshake, validate user access and create a JWT (JSON Web Token) to ensure user identity. A middleware is responsible to enforce user identity and roles on each request, depending on requested scopes of this request.
